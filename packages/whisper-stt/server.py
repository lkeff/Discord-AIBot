...
Endpoints:
  ...
  POST /transcribe/url  — transcribe audio at a given URL (optional)
"""
...
async def health() -> JSONResponse:
    ...
    try:
        model = get_model()
        return JSONResponse({"status": "ok", "model": MODEL_NAME, "device": str(model.device)})
...
async def transcribe(
    file: UploadFile = File(..., description="Audio file (mp3, wav, ogg, m4a, webm, …)"),
    language: Optional[str] = Form(None, description="ISO-639-1 language code, e.g. 'en'. Auto-detect if omitted."),
