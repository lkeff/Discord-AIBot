...
class UserSettings {
  static findOne(query) {
    ...
    return Promise.resolve(rowToObj(db.prepare('SELECT * FROM user_settings WHERE userId = ?').get(userId)));
  }

  static find() {
    return Promise.resolve(db.prepare('SELECT * FROM user_settings').all().map(rowToObj));
  }
  ...
  static findOneAndUpdate(filter, update, _options = {}) {
    ...
    const updates = Object.keys(payload).map((k) => `${k} = excluded.${k}`).join(', ');
    db.prepare(
