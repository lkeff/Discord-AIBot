...
async function gatherClientStates(clients, token) {
    const states = await Promise.all(clients.map(async (name) => {
...
function analyzePatterns(states) {
    const patterns = [];
...
function findSharedErrors(states) {
    if (states.length < 2)
        return [];
...
function buildUniversalFix(patterns, states) {
    const fixes = [];
...
export async function runGlobalSequentialThinking(goal, clients, token) {
...
//# sourceMappingURL=global-thinking.js.map