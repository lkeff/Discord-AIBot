...
server.registerTool("start_thinking_session", {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ sessionId: session.sessionId, status: session.status }, null, 2) }] };
});
server.registerTool("add_thinking_step", {
    ...
    if (!step)
        return { content: [{ type: "text", text: "Error: Could not add step." }] };
    return { content: [{ type: "text", text: JSON.stringify({ stepId: step.id, progress: `[${step.stepNumber}/${step.totalSteps}]`, title: step.title, confidence: step.confidence }, null, 2) }] };
});
server.registerTool("complete_thinking_session", {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ sessionId: session.sessionId, status: session.status, totalSteps: session.steps.length, summary: session.summary }, null, 2) }] };
});
server.registerTool("get_thinking_session", {
    ...
    if (!session)
        return { content: [{ type: "text", text: "Error: No session found." }] };
...
Token defaults to DISCORD_BOT_TOKEN env var if not provided.`,
    ...
    switch (params.type) {
        case "guild":
            if (!params.guildId)
                return { content: [{ type: "text", text: "Error: guildId required for type=guild" }] };
            result = await discord.fetchGuild(params.guildId, params.token);
            break;
        case "channels":
            if (!params.guildId)
                return { content: [{ type: "text", text: "Error: guildId required for type=channels" }] };
            result = await discord.fetchGuildChannels(params.guildId, params.token);
            break;
        case "messages":
            if (!params.channelId)
                return { content: [{ type: "text", text: "Error: channelId required for type=messages" }] };
            result = await discord.fetchMessages(params.channelId, params.limit, params.token);
            break;
        case "members":
            if (!params.guildId)
                return { content: [{ type: "text", text: "Error: guildId required for type=members" }] };
            result = await discord.fetchGuildMembers(params.guildId, params.limit, params.token);
            break;
        case "roles":
            if (!params.guildId)
                return { content: [{ type: "text", text: "Error: guildId required for type=roles" }] };
...
server.registerTool("edit_discord_data", {
    ...
    if (params.type === "channel") {
        if (!params.channelId)
            return { content: [{ type: "text", text: "Error: channelId required" }] };
        ...
        if (!params.channelId || !params.messageId)
            return { content: [{ type: "text", text: "Error: channelId and messageId required" }] };
        ...
        if (!params.guildId || !params.userId)
            return { content: [{ type: "text", text: "Error: guildId and userId required for member" }] };
        result = await discord.editGuildMember(params.guildId, params.userId, params.data, params.token);
    }
    return { content: [{ type: "text", text: JSON.stringify({ ok: result.ok, status: result.status, data: result.data, error: result.error }, null, 2) }] };
});
server.registerTool("move_discord_data", {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ moved: results.length, results }, null, 2) }] };
});
server.registerTool("delete_discord_data", {
    ...
    if (params.type === "message") {
        ...
        if (!params.channelId || !params.messageIds?.length)
            return { content: [{ type: "text", text: "Error: channelId and messageIds required" }] };
    ...
    return { content: [{ type: "text", text: JSON.stringify({ ok: result.ok, status: result.status, error: result.error }, null, 2) }] };
});
// ═══════════════════════════════════════════════════════════════════
...
server.registerTool("scan_project_files", {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ total: files.length, files: summary }, null, 2) }] };
});
...
server.registerTool("list_containers", {
    ...
    return { content: [{ type: "text", text: output || "No containers found or Docker not accessible" }] };
});
// ═══════════════════════════════════════════════════════════════════
