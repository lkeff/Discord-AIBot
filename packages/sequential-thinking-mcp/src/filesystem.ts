...
export async function scanProjectFiles(
  rootPath: string,
  types: "markdown" | "todo" | "plan" | "all" | "source",
  includeContent = true
): Promise<FileEntry[]> {
  const filter = (f: string): boolean => {
    ...
    if (types === "markdown") return ext === ".md";
    if (types === "todo") return base.includes("todo") || base.includes("task");
    if (types === "plan") return base.includes("plan") || base.includes("deployment") || base.includes("checklist");
    if (types === "source") return [".ts", ".js"].includes(ext);
...
export async function analyzeSourceFile(filePath: string): Promise<RefactorSuggestion[]> {
...
export function formatData(
  data: unknown,
  format: "json" | "yaml" | "table" | "markdown" | "summary"
): string {
  if (format === "json") {
  ...
  if (format === "table") {
    if (!Array.isArray(data) || data.length === 0) return "No tabular data";
  ...
  if (format === "yaml") {
    ...
    function toYaml(val: unknown, indent = 0): string {
      ...
      if (Array.isArray(val)) return val.map((v) => `${pad}- ${toYaml(v, indent + 2)}`).join("\n");
