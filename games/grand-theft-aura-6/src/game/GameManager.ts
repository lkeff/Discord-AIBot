...
export class GameManager {
  ...
  private playerStats: Map<string, { totalKills: number; totalDeaths: number; totalAura: number; gamesPlayed: number }> = new Map();

  createGame(channelId: string): boolean {
  ...
  async startGame(channelId: string, channel: TextChannel): Promise<boolean> {
    const game = this.games.get(channelId);
    if (!game || game.isActive || game.players.size < 2) return false;
  ...
  private async sendGameStartEmbed(channel: TextChannel, game: Game, slasher: Player) {
  ...
  private startRoundTimer(channel: TextChannel, game: Game) {
  ...
  async attackPlayer(channelId: string, attackerId: string, targetId: string, channel: TextChannel): Promise<string | null> {
    const game = this.games.get(channelId);
    ...
    if (!attacker || !target) return 'Invalid players!';
    if (!target.isAlive) return 'Target is already dead!';
    if (attackerId === targetId) return 'You cannot attack yourself!';
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${target.username}** has been brutally eliminated!\n\n` +
  ...
  private async endRound(channel: TextChannel, game: Game, slasherWon: boolean) {
    if (game.roundTimer) {
  ...
  getAllPlayersStats(): Map<string, any> {
